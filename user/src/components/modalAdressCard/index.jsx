...
export default function ModalAdressCard({
  ...
  async function handleSendAdress(e) {
    ...
    try {
      await axiosPrivate.post(`/adress/${await AsyncStorage.getItem('usuarioId')}`, { line_1: `${adress.number}, ${adress.street},  ${adress.neighborhood}`, line_2: adress.complement, city: adress.city, state: adress.state, country: adress.country, zip_code: adress.zip_code }, {
        headers: {
          'Authorization': `Bearer ${await AsyncStorage.getItem("token")}`
        }
      });

      toastSuccess('Endereço cadastrado com sucesso!', 3000, 'top-left')
      setTimeout(() => {
        setShowModal("");
      }, 3000)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col justify-center items-center bg-bgModal absolute top-0 w-full h-full">
      <div
        className={`relative flex flex-col justify-evenly w-1/3 ${type === "Endereço" ? "h-2/4" : "h-3/5"
          ...
          <div>
            <div className="w-full flex items-end gap-4">
              <Input
                ...
                set={(e) => { setAdress({ ...adress, zip_code: e.target.value }) }}
                value={adress.zip_code}
              />
              <Button onClick={() => getAdressViaCep(adress.zip_code)} className='bg-green rounded-3xl w-full p-2' text='Preencher endereço com CEP' type='button' />
            </div>
            <div className="flex gap-6">
              <Input
                ...
                set={(e) => { setAdress({ ...adress, street: e.target.value }) }}
                value={adress.street}
                className="w-3/4"
              />
              <Input
                ...
                type="number"
                placeholder="Número"
                set={(e) => { setAdress({ ...adress, number: e.target.value }) }}
                value={adress.number}
                className="w-1/4"
              />
            </div>
            <div className="flex gap-6">
              <Input
                ...
                set={(e) => { setAdress({ ...adress, complement: e.target.value }) }}
              ...
              <Input
                ...
                type="text"
                placeholder="Bairro"
                set={(e) => { setAdress({ ...adress, neighborhood: e.target.value }) }}
            ...
            <div className="flex gap-6">
              <Input
                ...
                type="text"
                placeholder="Cidade"
                set={(e) => { setAdress({ ...adress, city: e.target.value }) }}
                value={adress.city}
                className="w-1/2"
              />
              <Input
                ...
                type="text"
                placeholder="Estado"
                set={(e) => { setAdress({ ...adress, state: e.target.value }) }}
                value={adress.state}
                maxLength={2}
                className="w-1/4"
              />
              <Input
                ...
                type="text"
                placeholder="Pais"
                set={(e) => { setAdress({ ...adress, country: e.target.value }) }}
