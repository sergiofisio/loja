...
export default function ModalUserHeader({
  setShowModalHeader,
  setAdmin,
}: {
  setShowModalHeader: React.Dispatch<React.SetStateAction<boolean>>;
  setAdmin: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  const navigate = useNavigate();
  const location = useLocation();
  function handleMeusPedidos(e: React.MouseEvent<HTMLElement>) {
    e.preventDefault();
    e.stopPropagation();
  ...
  function handleSair(e: React.MouseEvent<HTMLElement>) {
    e.preventDefault();
    e.stopPropagation();
